...
int main(int argc, char **argv){
    ...
    const int index_len = cache_size*1024 / block_size;
    if(associativity == 0)
    ...
    const int offset_bit = log2(block_size);
    const int tag_bit = 32 - offset_bit - index_bit;
    ...
    while(fin >> hex >> temp){
        ...
        if(associativity == 0){ //1-way
            ...
            }else{ //miss, My Policy(replace the last find one)
